...
def _ensure_placeholder_dirs(rctx, add_prefix):
    for exclude in rctx.attr.excludes:
        ...
        keep_file = directory + "/.http_bsdtar_archive.keep"
        rctx.file(keep_file, "")
...
def _http_bsdtar_archive_impl(rctx):
    ...
    keep_file = None
    if add_prefix:
        keep_file = add_prefix + "/.http_bsdtar_archive.keep"
        rctx.file(keep_file, "")
...
http_bsdtar_archive = repository_rule(
    implementation = _http_bsdtar_archive_impl,
    attrs = _http_bsdtar_archive_attrs,
