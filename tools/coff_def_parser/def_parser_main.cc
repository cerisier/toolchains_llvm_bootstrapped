...
#ifdef _WIN32
  std::wstring filenameW = AsAbsoluteWindowsPath(filename);
  return _wfopen(filenameW.c_str(), L"w+");
#else
  return fopen(filename, "w+");
#endif
}

std::ifstream OpenParamFile(const char* filename) {
#ifdef _WIN32
  std::wstring filenameW = AsAbsoluteWindowsPath(filename);
  return std::ifstream(filenameW.c_str(), std::ios::in | std::ios::binary);
#else
  return std::ifstream(filename, std::ios::in | std::ios::binary);
#endif
}
...
int main(int argc, char* argv[]) {
  ...
  FILE* fout = OpenOutputFile(argv[1]);
  if (!fout) {
  ...
  for (int i = 3; i < argc; i++) {
    ...
    if (argv[i][0] == '@') {
      std::ifstream paramfile = OpenParamFile(argv[i] + 1);
      if (!paramfile) {
