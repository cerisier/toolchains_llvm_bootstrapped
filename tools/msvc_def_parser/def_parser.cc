...
constexpr WORD IMAGE_FILE_MACHINE_ARMNT = 0x01c4;
constexpr WORD IMAGE_FILE_MACHINE_AMD64 = 0x8664;
constexpr WORD IMAGE_FILE_MACHINE_ARM64 = 0xaa64;
constexpr WORD IMAGE_FILE_MACHINE_ARM64EC = 0xa641;
constexpr BYTE IMAGE_SYM_CLASS_EXTERNAL = 2;
...
constexpr size_t IMAGE_SIZEOF_FILE_HEADER = 20;
...
static_assert(sizeof(IMAGE_FILE_HEADER) == 20, "Unexpected COFF header size");
static_assert(sizeof(IMAGE_SECTION_HEADER) == 40, "Unexpected COFF section header size");
static_assert(sizeof(IMAGE_SYMBOL) == 18, "Unexpected COFF symbol size");
#endif
...
public:
    ...
    , DataSymbols(dataSymbols)
  {
    ...
    this->SectionCount = this->ObjectImageHeader->NumberOfSections;
   ...
   *----------------------------------------------------------------------
   */
  bool DumpObjFile() { return this->DumpExternalsObjects(); }
  ...
  bool DumpExternalsObjects()
  {
    ...
    for (i = 0; i < this->SymbolCount; i++) {
      if (pSymbolTable->SectionNumber > 0 &&
          static_cast<DWORD_PTR>(pSymbolTable->SectionNumber) >
            this->SectionCount) {
        return false;
      }
      if (pSymbolTable->SectionNumber > 0 &&
...
#else
  if (errno != 0) {
    std::cerr << "(error: " << errno << "): " << std::strerror(errno) << "\n";
  }
#endif
}
...
static bool HasBytes(size_t fileSize, size_t offset, size_t count)
{
  return offset <= fileSize && count <= fileSize - offset;
}

static std::ifstream OpenInputFile(const char* filename)
{
#ifdef _WIN32
...
static bool ReadFile(const char* filename, std::vector<char>& contents)
{
  std::ifstream input = OpenInputFile(filename);
  if (!input) {
    PrintLastError();
                     ...
                     std::set<std::string>& dataSymbols)
{
  ...
  const size_t fileSize = contents.size();
  if (!HasBytes(fileSize, 0, sizeof(IMAGE_DOS_HEADER))) {
    fprintf(stderr, "Object file '%s' is too small\n", filename);
    return false;
  }
  void* lpFileBase = contents.data();
  ...
  if (dosHeader->e_magic == IMAGE_DOS_SIGNATURE) {
    ...
    if (!HasBytes(fileSize, 0, sizeof(IMAGE_FILE_HEADER))) {
    ...
    if (((imageHeader->Machine == IMAGE_FILE_MACHINE_I386) ||
      ...
      size_t sectionHeadersOffset =
        IMAGE_SIZEOF_FILE_HEADER + imageHeader->SizeOfOptionalHeader;
      size_t sectionHeadersSize =
        static_cast<size_t>(imageHeader->NumberOfSections) *
        sizeof(IMAGE_SECTION_HEADER);
      size_t symbolTableSize =
        static_cast<size_t>(imageHeader->NumberOfSymbols) *
        sizeof(IMAGE_SYMBOL);
      if (!HasBytes(fileSize, sectionHeadersOffset, sectionHeadersSize) ||
          !HasBytes(fileSize, imageHeader->PointerToSymbolTable,
                    symbolTableSize)) {
        fprintf(stderr, "Object file '%s' has truncated COFF tables\n",
                filename);
        return false;
      }
      ...
      if (HasBytes(fileSize, 0, sizeof(cmANON_OBJECT_HEADER_BIGOBJ)) &&
          h->Sig1 == 0x0 && h->Sig2 == 0xffff) {
        size_t sectionHeadersOffset = sizeof(cmANON_OBJECT_HEADER_BIGOBJ);
        size_t sectionHeadersSize =
          static_cast<size_t>(h->NumberOfSections) *
          sizeof(IMAGE_SECTION_HEADER);
        size_t symbolTableSize =
          static_cast<size_t>(h->NumberOfSymbols) *
          sizeof(cmIMAGE_SYMBOL_EX);
        if (!HasBytes(fileSize, sectionHeadersOffset, sectionHeadersSize) ||
            !HasBytes(fileSize, h->PointerToSymbolTable, symbolTableSize)) {
          fprintf(stderr, "Object file '%s' has truncated bigobj tables\n",
                  filename);
          return false;
        }
        // bigobj
        ...
        if (!symbolDumper.DumpObjFile()) {
          fprintf(stderr,
                  "Object file '%s' has an invalid section reference\n",
                  filename);
          return false;
        }
      } else {
